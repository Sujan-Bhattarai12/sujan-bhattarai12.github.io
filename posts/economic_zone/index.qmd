---
title: 'Exploring Optimal Locations for Marine Aquaculture: A Comprehensive Analysis'
author: "Sujan"
format:
  html:
    code-fold: true
...
Marine aquaculture stands at the forefront of sustainable solutions for meeting the growing global demand for protein. In comparison to conventional land-based meat production, marine aquaculture offers a more eco-friendly approach. Previous research, such as the work by [Gentry et al. (2017)](https://www.nature.com/articles/s41559-017-0257-9), has mapped the global potential for marine aquaculture, considering various constraints. In this project, our focus narrows to the Exclusive Economic Zones (EEZ) along the West Coast of the United States, aiming to identify the most suitable regions for developing marine aquaculture, specifically for various species of oysters.
...
```{r include=TRUE, warning=FALSE, messages=FALSE, collapse=TRUE}
#| fig-cap: "East Coast Region with Five Sub-division"
##economic region data
west_coast_shape <- st_read("data/data/wc_regions_clean.shp")
plot(west_coast_shape['rgn_id'], main = "West Coast")
```
...
```{r include=TRUE, message=FALSE}
#| code-fold: true
#| fig-cap: "Percentage area in Each region in West Coast"
...
```{r}
#| code-fold: true
#| fig-cap: "East Coast Region with basemap for regions around it"
...
#### Function that create EEZ for your species
```{r, collapse=TRUE}
